...
type CustomerQuery struct {
	...
	// eager-loading edges.
	withFormcustomer *OrderonlineQuery
...
// QueryFormcustomer chains the current query on the formcustomer edge.
func (cq *CustomerQuery) QueryFormcustomer() *OrderonlineQuery {
	query := &OrderonlineQuery{config: cq.config}
...
//  WithFormcustomer tells the query-builder to eager-loads the nodes that are connected to
// the "formcustomer" edge. The optional arguments used to configure the query builder of the edge.
func (cq *CustomerQuery) WithFormcustomer(opts ...func(*OrderonlineQuery)) *CustomerQuery {
	query := &OrderonlineQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withFormcustomer = query
	return cq
}
...
func (cq *CustomerQuery) sqlAll(ctx context.Context) ([]*Customer, error) {
	...
	if query := cq.withFormcustomer; query != nil {
		...
		query.Where(predicate.Orderonline(func(s *sql.Selector) {
			s.Where(sql.InValues(customer.FormcustomerColumn, fks...))
		}))
