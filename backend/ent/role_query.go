...
type RoleQuery struct {
	...
	// eager-loading edges.
	withTodo *EmployeeWorkingHoursQuery
...
func (rq *RoleQuery) QueryTodo() *EmployeeWorkingHoursQuery {
	query := &EmployeeWorkingHoursQuery{config: rq.config}
...
func (rq *RoleQuery) WithTodo(opts ...func(*EmployeeWorkingHoursQuery)) *RoleQuery {
	query := &EmployeeWorkingHoursQuery{config: rq.config}
...
func (rq *RoleQuery) sqlAll(ctx context.Context) ([]*Role, error) {
	...
	if query := rq.withTodo; query != nil {
		...
		query.Where(predicate.EmployeeWorkingHours(func(s *sql.Selector) {
			s.Where(sql.InValues(role.TodoColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.role_todo
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "role_todo" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "role_todo" returned %v for node %v`, *fk, n.ID)
			}
