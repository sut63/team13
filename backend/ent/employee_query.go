...
type EmployeeQuery struct {
	...
	// eager-loading edges.
	withWhose         *EmployeeworkinghoursQuery
	withEmployeestock *StockQuery
	withFormemployee  *SalaryQuery
...
// QueryWhose chains the current query on the whose edge.
func (eq *EmployeeQuery) QueryWhose() *EmployeeworkinghoursQuery {
	query := &EmployeeworkinghoursQuery{config: eq.config}
...
// QueryEmployeestock chains the current query on the employeestock edge.
func (eq *EmployeeQuery) QueryEmployeestock() *StockQuery {
...
// QueryFormemployee chains the current query on the formemployee edge.
func (eq *EmployeeQuery) QueryFormemployee() *SalaryQuery {
...
//  WithWhose tells the query-builder to eager-loads the nodes that are connected to
// the "whose" edge. The optional arguments used to configure the query builder of the edge.
func (eq *EmployeeQuery) WithWhose(opts ...func(*EmployeeworkinghoursQuery)) *EmployeeQuery {
	query := &EmployeeworkinghoursQuery{config: eq.config}
...
//  WithEmployeestock tells the query-builder to eager-loads the nodes that are connected to
// the "employeestock" edge. The optional arguments used to configure the query builder of the edge.
func (eq *EmployeeQuery) WithEmployeestock(opts ...func(*StockQuery)) *EmployeeQuery {
	query := &StockQuery{config: eq.config}
	for _, opt := range opts {
		opt(query)
	}
	eq.withEmployeestock = query
	return eq
}

//  WithFormemployee tells the query-builder to eager-loads the nodes that are connected to
// the "formemployee" edge. The optional arguments used to configure the query builder of the edge.
func (eq *EmployeeQuery) WithFormemployee(opts ...func(*SalaryQuery)) *EmployeeQuery {
	query := &SalaryQuery{config: eq.config}
	for _, opt := range opts {
		opt(query)
	}
	eq.withFormemployee = query
	return eq
}
...
func (eq *EmployeeQuery) sqlAll(ctx context.Context) ([]*Employee, error) {
	var (
		...
		loadedTypes = [3]bool{
			eq.withWhose != nil,
			eq.withEmployeestock != nil,
			eq.withFormemployee != nil,
		}
	)
	...
	if query := eq.withWhose; query != nil {
		...
		query.Where(predicate.Employeeworkinghours(func(s *sql.Selector) {
			s.Where(sql.InValues(employee.WhoseColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.employee_whose
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "employee_whose" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "employee_whose" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Whose = append(node.Edges.Whose, n)
		}
	}

	if query := eq.withEmployeestock; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Employee)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.Stock(func(s *sql.Selector) {
			s.Where(sql.InValues(employee.EmployeestockColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.employee_employeestock
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "employee_employeestock" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "employee_employeestock" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Employeestock = append(node.Edges.Employeestock, n)
		}
	}

	if query := eq.withFormemployee; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Employee)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.Salary(func(s *sql.Selector) {
			s.Where(sql.InValues(employee.FormemployeeColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.employee_formemployee
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "employee_formemployee" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
